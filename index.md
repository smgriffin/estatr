...
> boundary geometry are all in place.
...
- [Large pulls, pagination and
  resuming](#large-pulls-pagination-and-resuming)
...
`estatr` reads the key from the `ESTAT_API_KEY` environment variable, so
you can also set it in `.Renviron` yourself. The key is a secret: never
commit it or paste it into an issue. `estatr` redacts it from error
messages and test fixtures.
...
is the interactive-friendly search. It renames e-Stat’s raw columns to
stable snake_case and moves the useful ones to the front.
...
Japanese keywords work too, and you can restrict to recently updated
tables:
...
The `id` column is the `statsDataId` you pass to
...
searches e-Stat’s catalog of *downloadable files* (Excel/CSV/PDF) rather
than machine-readable data. Use it for discovery; use
[`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md)
...
[`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md)
is the main entry point. It fetches the data *and* its classification
metadata in a single call, decodes every numeric code to a label, and
...
Omit `limit` to fetch every matching row; `estatr` paginates
automatically.

## Understanding the output

Every classification axis comes back as a **pair** of columns: the
decoded label and the original code.
...
- `value` — **numeric**
- `annotation` — non-numeric markers preserved instead of silently
  becoming `NA`

### Annotations, not silent NAs

e-Stat uses markers like `-`, `***` and `*` for suppressed or
inapplicable cells. `estatr` keeps them:
...
The legend for those markers travels with the data as a `notes`
attribute:
...
For the handful of tables most people want first, you don’t need an id
at all.
...
Each has a dedicated wrapper, and all of them forward `...` to
[`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md):
...
Before pulling a big table, look at what axes and codes it has. This is
how you learn which `cd*` filter values are valid.
...
The `level` and `parent` columns describe the hierarchy — national →
...
`estatr` works in **English by default** — e-Stat provides the
translations.
...
Tables with no English release fall back to Japanese automatically and
warn, so English mode never returns blanks:
...
`estatr` downloads official e-Stat census boundary polygons and joins
them on `area_code`, giving you a choropleth-ready `sf` object. Requires
`sf`.

The shortcut is `geometry = TRUE` on
[`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md):
...
That `lvArea = "2"` matters. Most e-Stat tables stack every geography in
one table — national total, prefectures, municipalities, wards.
`0003433219` holds **1,965 areas**. Fetch all of them and join at
`level = "prefecture"` and only 47 get a polygon; the rest come back
with empty geometry and a warning.

e-Stat’s `lv<axis>` parameters filter by level server-side, and pass
straight through `...`:
...
For this table the levels are `1` national, `2` prefecture, `4` city,
`5` ward, `6` town/village. They vary by table, so check the `level` and
`parent` columns of `estat_meta_info(id)$area` rather than assuming. The
same works for other axes (`lvCat01`, `lvTime`, …).
...
Three levels are available, all derived from the same authoritative
small-area shapefiles so the codes always match your data:
...
- **Match `year` to your data’s census year.** Municipality codes change
  between censuses (mergers), so a mismatched year mis-joins.
...
- **Boundary name columns are Japanese only** (`PREF_NAME`, `CITY_NAME`)
  — the shapefiles ship no English. Your
  [`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md)
  labels are still English.
- **The national total (`00000`) has no polygon.** It is kept with an
  empty geometry rather than dropped, so row counts line up with the
  input.
- **Designated cities:** e-Stat’s shapefiles carry only ward codes. The
  default `designated_cities = "both"` returns ward polygons *and* a
  unioned parent-city polygon, so data coded at either level joins. Use
  `"ward"` or `"city"` to pick one.
- Downloads are cached, and all 47 prefectures is roughly 250 MB — pass
  `areas` to fetch only what you need.
...
e-Stat caps a single response at 100,000 records.
[`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md)
paginates automatically, fetching pages in parallel with bounded
concurrency and a polite client-side throttle.
...
For very large pulls, `checkpoint` makes the job resumable — rerun the
same call after an interruption and it picks up where it left off:
...
Internals use `data.table`; the tibble conversion happens only at the
return boundary. Two escape hatches for bulk work:
...
Classification metadata is cached on disk (with an in-session memoise
layer on top), and boundary downloads are cached too.
...
If you want the API surface as-is, each endpoint has a thin wrapper.
These mirror e-Stat’s own parameter and column names.
...
returns `value` as **character** (undecoded);
[`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md)
is what splits it into a numeric `value` plus `annotation`.

## Function reference

| Function | Purpose |
|----|----|
| [`get_estat()`](https://smgriffin.github.io/estatr/reference/get_estat.md) | Main entry point: tidy, labelled data |
| [`search_estat()`](https://smgriffin.github.io/estatr/reference/search_estat.md) | Interactive catalog search |
| [`estat_meta_info()`](https://smgriffin.github.io/estatr/reference/estat_meta_info.md) | A table’s classification axes and codes |
| [`pivot_estat_wide()`](https://smgriffin.github.io/estatr/reference/pivot_estat_wide.md) | Reshape tidy output to wide form |
| [`estat_curated_tables()`](https://smgriffin.github.io/estatr/reference/estat_curated_tables.md) | List the curated shortcut tables |
| [`get_estat_curated()`](https://smgriffin.github.io/estatr/reference/get_estat_curated.md) | Fetch a curated table by key |
| [`get_labour_force_survey()`](https://smgriffin.github.io/estatr/reference/get_labour_force_survey.md) | Curated: Labour Force Survey |
| [`get_population_census()`](https://smgriffin.github.io/estatr/reference/get_population_census.md) | Curated: Population Census 2020 |
| [`get_family_income_survey()`](https://smgriffin.github.io/estatr/reference/get_family_income_survey.md) | Curated: Family Income and Expenditure Survey |
| [`get_economic_census()`](https://smgriffin.github.io/estatr/reference/get_economic_census.md) | Curated: Economic Census 2021 |
| [`estat_boundaries()`](https://smgriffin.github.io/estatr/reference/estat_boundaries.md) | Download boundary polygons as `sf` |
| [`estat_join_geometry()`](https://smgriffin.github.io/estatr/reference/estat_join_geometry.md) | Join polygons onto e-Stat data |
| [`estat_api_key()`](https://smgriffin.github.io/estatr/reference/estat_api_key.md) | Register your appId |
| [`estat_api_key_exists()`](https://smgriffin.github.io/estatr/reference/estat_api_key_exists.md) | Check for a registered key |
| [`estat_cache_dir()`](https://smgriffin.github.io/estatr/reference/estat_cache_dir.md) | Where the cache lives |
| [`estat_cache_clear()`](https://smgriffin.github.io/estatr/reference/estat_cache_clear.md) | Clear cached metadata/checkpoints |
| [`estat_stats_list()`](https://smgriffin.github.io/estatr/reference/estat_stats_list.md) | Low-level `getStatsList` |
| [`estat_stats_data()`](https://smgriffin.github.io/estatr/reference/estat_stats_data.md) | Low-level `getStatsData` |
| [`estat_data_catalog()`](https://smgriffin.github.io/estatr/reference/estat_data_catalog.md) | Low-level `getDataCatalog` |
