...
- `estat_stats_data()` → wraps `getStatsData`, with **automatic, parallelized pagination** baked in: use the total record count from page one to fan out remaining pages concurrently (see Performance guidelines above) rather than the naive sequential `NEXT_KEY` loop — this is a real improvement over `estatapi`, which leaves pagination to the user.
  - **Gate findings (verified during M2 build, 2026-07-03):** (1) `startPosition` **is** a true absolute row offset — page[1:2]+page[3:4] equals full[1:4] against live data, so parallel offset fan-out is valid. (2) httr2 1.1.1's `req_perform_parallel()` does **not** respect `local_mocked_responses` — it hits the real network — so the offline mock seam is the internal `estat_fetch_bodies()`/`estat_perform_many()` function (mocked via `testthat::local_mocked_bindings`), not the HTTP layer. (3) Bonus: `getStatsData` bundles its own `CLASS_INF`, so `get_estat()` fetches data+metadata in one call.
